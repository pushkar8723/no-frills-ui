...
<Meta title='Components/Dialog' compopent={Dialog}/>

# Dialog

I deviated from how most other libraries manages dialogs. This Dialog component has no prop called `open`. Instead it exposes
`open` and `close` methods. To call this method, ref to the `Dialog` component needs to be used. This is because of my observation
that often we need dialogs only to show or get some small amount of information asynchronously. But to maintain
this asynchronous behaviour we have to juggle a lot with the state in the application and have to deal with multiple functions.
...
I exploited this feature to create components for following common use cases to use a dialog.

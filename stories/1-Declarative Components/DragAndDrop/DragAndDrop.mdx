...
The `props` and `ref` to `DragAndDrop` is forwarded to the native `HTMLDivElement`.
Following are the props that are added on top of `HTMLDivElement`.
...
<DragAndDrop
    ...
    movedAnnouncementTemplate="Elemento movido a posición {:position}"
/>
```
